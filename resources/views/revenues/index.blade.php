...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl text-card-foreground mb-2">Faturamento</h1>
            <p class="text-muted-foreground">Gerencie suas receitas</p>
        </div>
        <button onclick="document.getElementById('revenueForm').classList.toggle('hidden')"
            ...
            <x-icon name="plus" class="w-4 h-4" /> Nova Receita
        </button>
    </div>

    <div class="bg-card border border-border rounded-lg p-6">
        <div class="text-sm text-muted-foreground mb-2">Total de Receitas</div>
        <div class="text-4xl text-primary">R$ {{ number_format($total, 2, ',', '.') }}</div>
    </div>

    <div id="revenueForm" class="hidden bg-card border border-border rounded-lg p-6">
        <h3 class="text-lg text-card-foreground mb-4">Nova Receita</h3>
        <form method="POST" action="{{ route('revenues.store') }}" class="grid gap-4 md:grid-cols-2">
            @csrf
            ...
            <div class="md:col-span-2 flex gap-2">
                ...
                <button type="button" onclick="document.getElementById('revenueForm').classList.add('hidden')"
                ...
                <tbody>
                    @forelse ($revenues as $revenue)
                        <tr class="border-t border-border">
                            <td class="px-6 py-4 text-card-foreground">{{ $revenue->description }}</td>
                            <td class="px-6 py-4 text-muted-foreground">{{ $revenue->category }}</td>
                            <td class="px-6 py-4 text-muted-foreground">{{ $revenue->date->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 text-right text-primary">R$ {{ number_format($revenue->value, 2, ',', '.') }}</td>
